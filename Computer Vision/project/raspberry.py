...
saved_ids = set()  # Salva gli ID già scritti
...
while True:
    ...
    for result in results:
        ...
        for box in result.boxes:
            ...
            conf = box.conf[0]
            threshold = confidence_thresholds.get(class_name, 1.0)  # Default threshold alto per non salvare classi non interessanti
            ...
            if class_name in classes_of_interest:
                detection_id = box.id[0].item() if hasattr(box, 'id') else None
                
                # Salva solo se ID non è già presente
                if detection_id is not None and detection_id not in saved_ids:
